...
class State {
    ...
    draw() {
        // ボードの中身を数値化してホイ
        let numBoard = new Array(State.height);
        for (let i = 0; i < State.height; ++i) {
...
class BoardCell {
    ...
    // 石の色を変える
    setColor(y, x, cell) {
        let color = BoardCell.backgroundColor;
...
class Player {
    ...
    getPos(clientY, clientX, offsetY, offsetX) {
...
class Game {
    ...
    move(y, x) {
        const result = this.players[this.turn].move(y, x);
        if (this.isFinished()) {
            this.draw(true);
            return true;
        }
